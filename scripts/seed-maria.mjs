...
if (!URL || !SERVICE_KEY || !ANON_KEY) { console.error("Missing Supabase env vars."); process.exit(1) }
...
const BATCHES = [
  { key: "japan-may", title: "Japan Spring — May 2026", category: "Food & Beauty", startsOn: "2026-05-04", endsOn: "2026-05-14", reservationHours: 48, state: "live",
    notes: "Osaka + Tokyo run. Snacks, matcha, and drugstore beauty.",
    products: [ { name: "Royce Nama Chocolate", base: 520, markup: 100, qty: 10 }, { name: "Shiro Perfume", base: 1800, markup: 300, qty: 5 }, { name: "Cezanne BB Cream", base: 380, markup: 80, qty: 8 }, { name: "Calbee Jagabee", base: 180, markup: 40, qty: 12 } ] },
  { key: "korea-feb", title: "Korea Winter — February 2026", category: "Skincare", startsOn: "2026-02-05", endsOn: "2026-02-14", reservationHours: 48, state: "locked",
    notes: "CLOSED — trip already completed. Kept for records.",
    products: [ { name: "Round Lab Toner", base: 620, markup: 130, qty: 8 }, { name: "Torriden Serum", base: 750, markup: 150, qty: 8 }, { name: "Medicube Zero Pad", base: 890, markup: 160, qty: 6 } ] },
  { key: "japan-jan", title: "Japan New Year — January 2026", category: "Grocery & Snacks", startsOn: "2026-01-08", endsOn: "2026-01-16", reservationHours: 24, state: "expired",
    notes: "Claim window closed — unpaid reservations expired.",
    products: [ { name: "Shine Muscat Gummy", base: 300, markup: 70, qty: 10 }, { name: "Yokan Red Bean", base: 260, markup: 60, qty: 8 } ] },
]
...
const ORDERS = [
  ...
  { buyer: "trisha", batchKey: "japan-may", product: "Royce Nama Chocolate", qty: 2, advance: "pending" },
  ...
  { buyer: "carlo", batchKey: "korea-feb", product: "Round Lab Toner", qty: 1, advance: "completed", method: "GCash", historical: true },
  { buyer: "mia", batchKey: "japan-jan", product: "Shine Muscat Gummy", qty: 3, advance: "completed", method: "Bank Transfer", historical: true },
  { buyer: "grace", batchKey: "japan-jan", product: "Yokan Red Bean", qty: 2, advance: "completed", method: "GCash", historical: true },
  { buyer: "anna", batchKey: "japan-may", product: "Royce Nama Chocolate", qty: 1, advance: "completed", method: "Maya", historical: true },
  // Cancelled
  { buyer: "ben", batchKey: "japan-jan", product: "Shine Muscat Gummy", qty: 1, advance: "cancelled" },
]
...
async function reset() {
  ...
  const { data: batches } = await admin.from("batches").select("id").eq("seller_id", mariaId).in("title", titles)
  const ids = (batches ?? []).map((b) => b.id)
  if (ids.length) {
    const { data: prods } = await admin.from("batch_products").select("id").in("batch_id", ids)
    const pids = (prods ?? []).map((p) => p.id)
    if (pids.length) {
      const { data: ords } = await admin.from("orders").select("id").in("batch_product_id", pids)
...
async function alreadySeeded() {
  const mariaId = await idOf("maria")
  const { count } = await admin.from("batches").select("*", { count: "exact", head: true })
    .eq("seller_id", mariaId).eq("title", BATCHES[0].title)
  return (count ?? 0) > 0
}
...
async function seedBatches() {
  ...
  for (const b of BATCHES) {
    const { data: batch, error } = await maria.from("batches").insert({
      seller_id: mariaId, title: b.title, status: "live",
      starts_on: b.startsOn, ends_on: b.endsOn, category: b.category,
      reservation_hours: b.reservationHours, notes: b.notes ?? null,
    }).select("id").single()
    if (error) throw new Error(`batch ${b.title}: ${error.message}`)
    batchDbId.set(b.key, batch.id)
    const { data: products, error: pErr } = await maria.from("batch_products").insert(
...
async function seedOrders() {
  for (const o of ORDERS) {
    ...
    if (!productId) { console.warn(`  ! no product ${o.batchKey}::${o.product}`); continue }
    ...
    const buyerName = { trisha: "Trisha Lim", carlo: "Carlo Santos", anna: "Anna Bautista", mia: "Mia Cruz", ben: "Ben Torres", grace: "Grace Reyes" }[o.buyer] ?? "Buyer"
    const { data: payment, error: payErr } = await buyer.rpc("submit_order_payment", {
      p_order_id: orderId, p_method_type: o.method, p_amount: total,
      p_reference_number: `${o.method.slice(0, 3).toUpperCase()}-${orderId.slice(0, 8)}`,
      p_receipt_path: null, p_payer_account_name: buyerName, p_payer_phone: "0917-000-0000", p_buyer_contact_url: null,
    })
    ...
    if (o.advance === "completed") {
      ...
      if (o.historical) {
        ...
        await admin.from("orders").update({ payment_confirmed_at: when, completed_at: when }).eq("id", orderId)
      }
    }
...
async function applyBatchStates() {
  ...
  if (expiredProductId) {
    ...
    const { data: order, error } = await buyer.rpc("claim_batch_product", { p_batch_product_id: expiredProductId, p_quantity: 1 })
    if (!error && order) {
      const past = new Date(Date.now() - 5 * 86400_000).toISOString()
      await admin.from("orders").update({ reservation_expires_at: past }).eq("id", order.id)
      console.log('  ~ "Japan New Year — January 2026" now has an EXPIRED unpaid reservation')
    } else if (error) {
...
async function main() {
  ...
  else if (await alreadySeeded()) { console.log("Maria's extra batches already exist. Re-run with --reset to rebuild."); return }
