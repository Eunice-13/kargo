...
// ─── Types ───────────────────────────────────────────────────────────────────
type AppStage = "login" | "signup" | "app"
type UserInfo = { name: string; email: string; role: Role; bio?: string }
type Tab = "Dashboard" | "Batches" | "My Claims" | "Payments" | "Orders" | "Reports" | "Settings"
type Role = "Buyer" | "Seller"
type ClaimStatus = "Pending" | "Paid and Reserved" | "Expired" | "Cancelled" | "Insufficient Payment"
type ReportStatus = "Open" | "Under Review" | "Resolved"
type SettingsSection = "Profile" | "Linked Accounts" | "Notifications" | "Payment Methods" | "Security"
...
const navIntent: { batchId: number | null; sellerName: string | null } = {
  batchId: null,
  sellerName: null,
}

const RETRO_THEME = false
const INDIGO = RETRO_THEME ? "#A84D38" : "#191BA9"
const CREAM = RETRO_THEME ? "#FBF4E3" : "#F7F3F3"
const CYAN_L = RETRO_THEME ? "#DDE9E0" : "#DEF3FA"
const SKY = RETRO_THEME ? "#5B817A" : "#5CC2F2"
const CORAL = RETRO_THEME ? "#D7982F" : "#FF8A65"
const GREEN = RETRO_THEME ? "#507A69" : "#3FBF8F"
const AMBER = RETRO_THEME ? "#D59B2C" : "#FFC24B"

const TODAY = "Sep 10, 2026"

// ─── Category gradients ───────────────────────────────────────────────────────
const CAT_GRAD: Record<string, string> = RETRO_THEME
  ? {
...
// ─── Status colours ───────────────────────────────────────────────────────────
const STATUS_C: Record<ClaimStatus, { bg: string; text: string; dot: string }> = {
...
const REPORT_C: Record<ReportStatus, { bg: string; text: string }> = {
...
// ─── Pay Modal ────────────────────────────────────────────────────────────────
const PAY_METHODS = ["GCash", "Maya", "Bank Transfer", "Cash on Meetup"]
const SELLER_PAY_DETAILS: Record<string, { name: string; number: string }> = {
...
// ─── Static data (initial values — mutable state lives in App root) ───────────
interface BatchStoredProduct {
  name: string
  price: number
  qty: number
  claimed: number
...
type BatchType = BatchItem

const CLAIMS_INIT: ClaimRow[] = [
  {
    id: 1,
...
const KANBAN_COL_BG: Record<KanbanCol, string> = {
  Claimed: "#EEF0FF",
...
const FULFILLMENT_INIT: FulfillmentOrder[] = [
  {
    id: "ful-1",
    col: "Claimed",
    buyer: "Trisha Lim",
...
function FulfillmentLiveRegion({ text }: { text: string }) {
  return (
...
// ─── Reports ──────────────────────────────────────────────────────────────────
function Reports({ reports, setReports, orders, role }: SharedState) {
